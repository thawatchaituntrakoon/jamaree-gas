...
- **`erp.html`** — **แอปหลักที่ใช้งานจริง** ข้อมูลทั้งหมดของธุรกิจอยู่ที่นี่ที่เดียว (localStorage) — ไฟล์นี้คือ SSOT ตามที่อธิบายด้านล่าง แก้/เพิ่มโมดูลทั้งหมดทำที่ไฟล์นี้ (ยกมาจากสวีท BizSuite ตอนเริ่มโปรเจกต์ พร้อมแบรนด์ JAMAREE GAS อยู่แล้ว) — บนเว็บที่ deploy แล้ว ลิงก์หลัก `/` เด้งเข้าไฟล์นี้ตรงๆ ผ่าน `_redirects` (ไม่มีหน้าแลนดิ้งเพจแยกแล้ว)
